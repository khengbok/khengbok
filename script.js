...
function getCategories(){
  ...
  // Eski haberlerde kullanılan hazır kategoriler kaybolmasın.
  const defaults=["BTS","BLACKPINK","Stray Kids","TWICE","IVE"];
  const result=[];
  [...defaults,...saved].forEach(c=>{
...
if(categoryGrid){
  ...
  if(!cats.length){
    categoryGrid.innerHTML='<div class="no-news"><h3>Henüz kategori yok 💜</h3><p>Yönetim panelinden kategori ekleyebilirsin.</p></div>';
  }else{
    cats.forEach(name=>{
